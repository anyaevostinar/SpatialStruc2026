...
#SBATCH --job-name=nssgd
#SBATCH -o nssgd%A_%a.out
...
mkdir -p nosyms-stress-grid-diff
cd nosyms-stress-grid-diff
...
args=" -START_MOI 0 -SPATIAL_STRUCT_MODE grid -ENABLE_STRESS 1"
./symbulation_sgp $args -SEED ${SLURM_ARRAY_TASK_ID} > run.log

## Run with sbatch -p facultynode --nodelist=edmonstone2024,margulis2024,carver,lederberg run-nosyms-stress-grid-diff.sh